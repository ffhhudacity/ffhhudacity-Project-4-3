...
// deleted "var randomPizzaContainerCss = null;" from the 1st submission.//
// resizePizzas(size) is called when the slider in the "Our Pizzas" section of the website moves.


// resizePizzas will be called when the slider in the "Our Pizzas" section changes.
var resizePizzas = function(size) { 
  window.performance.mark("mark_start_resize");   // User Timing API function

  // Changes the value for the size of the pizza above the slider
  // replace the querySelector with getElementById
  ...
  // original comment: "This function has been optimized so that instead of iterating through each pizza to change its size,
  // we're now updating the width of .randomPizzaContainer class itself, which triggers the Recalculate
  // Layout only once for all the random pizzas!" ---- not working well. 
  // iterates through pizza elements on teh page and changes their widths, instead.
  function changePizzaSizes(size) {
    ...
    // deleted "var newWidth = (pizzaContainer[0].offsetWidth + determineDx(pizzaContainer[0], size)) + "px";//

  //deleted "randomPizzaContainerCss.style.width = newWidth;//
  }
// deleted ";" on line 468//
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  /* OPTIMIZATION: I make the number of pizzas shown depending on teh window width.//
   */
...
// Below are deleted from the 1st version.
